...
export function serializeMapState() {
  return {
    version: 1,
    tiles: tileMap.map(row => [...row]),
    rooms: roomInstances.map(room => ({
      id: room.id,
      ...
      constructionProgress: room.constructionProgress ?? 1,
      constructionTime: room.constructionTime ?? 0,
      constructionTick: room.constructionTick ?? 0,
    })),
  };
}

export function loadMapState(snapshot) {
  clearMap();
  if (!snapshot) return;

  for (let y = 0; y < MAP_H; y++) {
    for (let x = 0; x < MAP_W; x++) {
      if (snapshot.tiles?.[y]?.[x] === 1) {
        tileMap[y][x] = 1;
        walkable[y][x] = true;
      }
    }
  ...
  let maxRoomId = -1;
  for (const savedRoom of snapshot.rooms || []) {
    const room = placeRoom(
      ...
      savedRoom.w,
      savedRoom.h,
      savedRoom.rotation || 0,
    );
    if (!room) continue;

    room.id = savedRoom.id;
    room.constructionProgress = savedRoom.constructionProgress ?? room.constructionProgress;
    room.constructionTime = savedRoom.constructionTime ?? room.constructionTime;
    room.constructionTick = savedRoom.constructionTick ?? room.constructionTick;
    maxRoomId = Math.max(maxRoomId, room.id);
  ...
  nextRoomId = Math.max(nextRoomId, maxRoomId + 1);
}
