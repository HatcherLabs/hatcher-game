...
function getVagueInsight(metric, value) {
  const phrases = {
    ...
    revenue: value > 500 ? 'Yield is picking up' : 'Yield is slow',
...
function getRoughInsight(metric, value) {
  ...
  const phrases = {
    ...
    revenue: `Roughly $${round(value)}/day yield`,
...
function getExactInsight(metric, value) {
  const phrases = {
    ...
    revenue: `$${Math.round(value).toLocaleString()}/day yield`,
...
function getFullInsight(metric, value) {
  ...
  const phrases = {
    ...
    revenue: `$${Math.round(value).toLocaleString()}/day yield — ${f.closeRate}% close rate`,
    costs: `$${Math.round(value).toLocaleString()}/day — Monthly projection: $${Math.round(value * 30).toLocaleString()}`,
  };
...
export function getOfficeCategoriesForCompany() {
  ...
  if (!roles) {
    ...
    return {
      delivery:   { label: 'Missions',       icon: '⚡', offices: ['seo','content','video','design','data','engineering','workshop'] },
  ...
  return {
    delivery:   { label: 'Missions',       icon: '⚡', offices: delivery },
    growth:     { label: 'Signal',         icon: '📈', offices: growth },
...
export function calculateACV() {
  ...
  if (companyPayMult !== 1.0) {
    acv *= companyPayMult;
    factors.push({ label: companyDef.name || 'Desk', mult: companyPayMult });
  }
