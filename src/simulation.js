        ...
        if (p.advancePhase(avgQuality + collabBonus)) {
          ...
          if (hasProductGrowth() && (p.targetOffice === 'engineering' || p.targetOffice === 'rd')) {
            ...
            if (G.productLevel >= 10 && G.productLevel - levelGain < 10) {
              showToast('🔧 Core Lv.10 — passive yield is starting to trickle in.');
            } else if (G.productLevel >= 50 && G.productLevel - levelGain < 50) {
              showToast('🚀 Core Lv.50 — passive yield now exceeds daily burn.');
            }
          }
          ...
          if (p.targetOffice === 'hr' && p.template.name === 'Hatch Cycle') {
...
function processDayCycle(dt) {
  ...
  if (G.dayTicks >= ECONOMY.day_ticks) {
    ...
    if (result === 'win') {
      ...
      showToast(`🎉 $1M YIELD! You built a thriving ${companyName.toLowerCase()}!`);
    } else if (result === 'need_loan') {
      G.gameSpeed = 0;
      showLoanModal();
    } else if (result === 'bankrupt') {
      sfxGameOver();
      showToast('💀 BANKRUPT! Your hub ran out of funds.');
    }
  ...
  if (countRoomsByType('rd') > 0) {
    ...
    if (G.rdBreakthroughTimer >= rdCycleWeeks) {
      ...
      if (staffed.length > 0) {
        ...
        showToast(`🧠 Core Breakthrough! ${roomDef?.icon || '🧠'} ${roomDef?.name || target} missions +20% pay for 2 weeks`);
      }
    }
  }
...
function spawnProjects(dt) {
  ...
  if (!G.starterProjectsGiven && G.agents.length > 0 && getRoomInstances().length > 1) {
    ...
    if (validTemplates.length > 0) {
      ...
      sfxFirstClients();
      showToast('🎉 First market flow arrived. 2 founding missions received.');
    }
  }
