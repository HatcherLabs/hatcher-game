// ═══════════════════════════════════════════════════════════════
//  HATCHER MARKETS — Build Mode (room placement, corridor painting)
// ═══════════════════════════════════════════════════════════════
...
export function hireAgent(roleKey, candidate = null) {
  ...
  if (G.money < signingCost) {
    sfxError();
    showToast('Not enough funds to hatch!');
    return false;
  }
  ...
  if (!hasOffice) {
    sfxError();
    showToast(`Build a ${OFFICE_TYPES[role.office]?.name || 'matching room'} first!`);
    return false;
  }
  ...
  showToast(`Hatched ${agent.name} as ${role.title}${salaryInfo}! -$${signingCost}`);
  G.uiDirty = true;
