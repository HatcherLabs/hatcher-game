// ═══════════════════════════════════════════════════════════════
//  HATCHER MARKETS — Auto Steward Mode
//  Autonomous play: builds rooms, hatches agents, manages the hub
// ═══════════════════════════════════════════════════════════════
...
export function toggleAiCeo() {
  aiEnabled = !aiEnabled;
  if (aiEnabled) {
    showToast('🤖 Auto Steward activated.');
    if (G.gameSpeed < 1) setSpeed(1);
  } else {
    showToast('🤖 Auto Steward deactivated. You are in control.');
  }
  return aiEnabled;
}
  ...
  for (const a of G.agents) {
    if (a.wantsRaise && a.roleKey !== 'ceo') {
      ...
      if (G.money > newSalary * 14) {
        ...
        showToast(`🤖 Auto Steward gave ${a.name} a raise: $${currentSalary} → $${newSalary}/day`);
        return true;
      }
    }
  }
...
function handleEquipment() {
  ...
  if (G.equipmentConfig.coffee_quality === 'instant' && G.money > 3000 && countRoomsByType('breakroom') > 0) {
    ...
    showToast('🤖 Auto Steward upgraded the Recharge Lounge.');
    changed = true;
  }
  ...
  if (G.equipmentConfig.sales_pricing === 'standard' && G.reputation > 60 && countRoomsByType('sales') > 0) {
    ...
    showToast('🤖 Auto Steward switched to premium routing.');
    changed = true;
  }
  ...
  if (countRoomsByType('shopfront') > 0 && G.equipmentConfig.pricing_strategy !== undefined) {
    if (G.equipmentConfig.pricing_strategy === 'standard' && G.reputation < 50) {
      ...
      showToast('🤖 Auto Steward set discount pricing for more foot traffic.');
      changed = true;
    } else if (G.equipmentConfig.pricing_strategy === 'discount' && G.reputation > 65) {
      G.equipmentConfig.pricing_strategy = 'standard';
      showToast('🤖 Auto Steward switched to standard pricing — reputation is strong.');
      changed = true;
      ...
      showToast('🤖 Auto Steward switched to premium pricing — market brand established.');
      changed = true;
    }
  }
...
function handleExpansions() {
  ...
  for (const exp of available) {
    ...
    if (canAfford) {
      ...
      showToast(`🤖 Auto Steward expanded: ${exp.name}! -$${cost.toLocaleString()} (50% off)`);
  ...
  for (const roomKey of buildOrder) {
    ...
    if (pos) {
      ...
      if (room) {
        ...
        sfxBuild();
        showToast(`🤖 Auto Steward built ${def.icon} ${def.name}! -$${cost.toLocaleString()}`);
...
function ensureRoomReachable(room) {
  ...
  if (painted > 0) {
    showToast(`🤖 Auto Steward built ${painted} corridor tiles to connect the new room.`);
  }
}
...
function handleTechTree() {
  ...
  for (const branch of preferred) {
    ...
    if (chooseTechBranch(branch)) {
      showToast(`🤖 Auto Steward chose branch: ${branch}!`);
      return true;
    }
  }
  return false;
...
function handleLoan() {
  if (G.money < 500 && G.money > -2000 && G.projects.length > 0 && G.loanCount < 3) {
    ...
    showToast(`🤖 Auto Steward took a $${loanAmount.toLocaleString()} loan to keep the hub alive.`);
    return true;
  }
  return false;
}
