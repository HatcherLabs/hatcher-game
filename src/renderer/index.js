...
function showGameOverOverlay() {
  ...
  const companyDef = COMPANY_TYPES[G.companyType] || { name: 'Hatcher Desk', icon: '🥚' };
  ...
  if (growthModel.model === 'exponential') {
    ...
    methodName = 'Yield Multiple (Protocol)';
    methodFormula = `ARR × ${multiple.toFixed(1)}x`;
    methodBreakdown = `
      <div class="go-calc-row"><span>Daily Passive Yield</span><span>$${mrr.toLocaleString()}/day</span></div>
    ...
    const modelLabel = growthModel.model === 'premium' ? 'Strategy'
      : growthModel.model === 'physical' ? 'Trade' : 'Desk';
    ...
    <div class="go-card">
      ...
      ${isWin ? `
        ...
        <div class="go-hero">
          ...
          <div class="go-hero-label">Founder Score</div>
        ...
        <div class="go-section">
          <div class="go-section-label">Hub Valuation</div>
        ...
        <!-- Equity Split -->
        <div class="go-section">
          <div class="go-section-label">Ownership</div>
          ...
          <div class="go-equity-legend">
            <span><span class="dot" style="background:#50c878"></span> Founder ${ceoStake}%</span>
            ${investorStake > 0 ? `<span><span class="dot" style="background:#e07030"></span> Backers ${investorStake}%</span>` : ''}
            ${esopStake > 0 ? `<span><span class="dot" style="background:#5090e0"></span> Agents ${esopStake}%</span>` : ''}
          </div>
          <div class="go-calc-row" style="margin-top:4px">
            <span>$${valuation.toLocaleString()} × ${ceoStake}% founder stake</span>
      ...
      <div class="go-stats">
        ...
        <div class="go-stat">
          ...
          <div class="go-stat-lbl">Missions</div>
        </div>
        <div class="go-stat">
          <div class="go-stat-val">${G.agents.length}</div>
          <div class="go-stat-lbl">Agents</div>
        </div>
      </div>
...
function getValuationLesson(model, ceoStake, roiMultiple) {
  ...
  if (model === 'exponential') {
    tips.push('<strong>Protocol desks</strong> score higher when passive yield compounds and growth stays strong.');
  } else if (model === 'premium') {
    tips.push('<strong>Strategy desks</strong> score on profit quality and reputation. Trust raises mission value.');
  } else if (model === 'physical') {
    tips.push('<strong>Trade desks</strong> score on margin discipline. Flow matters, but efficiency keeps the vault alive.');
  } else {
    tips.push('<strong>Signal desks</strong> score on steady profit and mission throughput.');
  }
  if (ceoStake < 60) {
    tips.push(`You kept only <strong>${ceoStake}%</strong> of the hub. Backers funded growth, but reduced founder upside.`);
  } else if (ceoStake === 100) {
    tips.push('You kept <strong>100%</strong> ownership. Maximum upside, but a tougher road.');
  }
  if (roiMultiple >= 50) {
    tips.push(`<strong>${roiMultiple}x return</strong> from the starting vault is a strong run.`);
  }
  return tips.join(' ');
}
...
function getBankruptLesson() {
  ...
  if (G.agents.length > 5 && G.day < 30) {
    tips.push('Hatching too fast burns cash before yield catches up. <strong>Stay lean early.</strong>');
  } else if (G.completedLog.length < 3) {
    tips.push('Too few missions completed. <strong>Focus on finishing missions</strong> to generate yield.');
  } else {
    tips.push('Watch your daily burn rate vs. yield. <strong>Profit = survival.</strong>');
  }
  return tips.join(' ');
}
