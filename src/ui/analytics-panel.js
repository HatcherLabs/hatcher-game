// ═══════════════════════════════════════════════════════════════
//  HATCHER MARKETS — Analytics Panel (insight display, levels 0-3)
// ═══════════════════════════════════════════════════════════════
...
export function updateAnalyticsPanel() {
  ...
  if (level === 0) {
    html += `
      ...
      <div class="analytics-hint">Build a Signal Lab to see real numbers.</div>
    `;
  } else if (level === 1) {
    html += `
      ...
      <div class="analytics-hint">Hatch a Signal Analyst for exact metrics.</div>
    `;
  } else if (level === 2) {
    html += `
    ...
    html += `
      <div class="analytics-card full">
        ...
        <div class="funnel">
          ...
          <div class="funnel-row">
            <span>💰 Yield</span>
            <span>${m.clients.toFixed(1)}/day · <strong style="color:var(--success)">$${(m.acv || 0).toLocaleString()}</strong> AVG</span>
          </div>
        ...
        <h4>Mission Value (AVG)</h4>
        <div class="metric-row" style="padding:5px 0">
          <span style="font-weight:600">Avg Mission Value</span>
        ...
        <div class="metric-row" style="padding:6px 0;border-top:1px solid var(--border);margin-top:4px">
          <span style="font-weight:700">Founder Score</span>
        ...
        <div style="font-size:10px;color:var(--text-faint);padding:2px 0">
          Valuation × ${G.capTable.ceo}% steward stake
        </div>
      </div>
    `;
  }
...
function getRecommendation() {
  const m = G.metrics;
  if (G.agents.length === 0) return '💡 Hatch your first agent to get started!';
  if (m.totalVisitors < 10) {
    if (G.companyType === 'fashion_retail') return '💡 Build more Market Stalls and hatch Market Clerks for walk-in traders.';
    return '💡 Build a Signal Relay for paid traffic or grow organic with Broadcast/Scanner rooms.';
  }
  if (m.leads > 5 && m.closeRate < 10) return '💡 Hatch a Deal Runner to improve your close rate.';
  if (m.contentQuality < 30) return '💡 Hatch a Lore Operator to improve organic traffic.';
  if (m.designQuality < 30) return '💡 A Blueprint Maker would boost your conversion rate.';
  if (G.agents.some(a => a.mood < 0.4)) return '💡 Your agents need a recharge. Build a Recharge Lounge.';
  const stalledCount = G.projects.filter(p => p.stalled).length;
  if (stalledCount > 2) return `💡 ${stalledCount} missions stalled — hatch more agents!`;
  return '📈 Market flow is healthy. Keep growing!';
}
...
function getGrowthModelCard() {
  ...
  if (gm.model === 'exponential') {
    // Protocol / Brain Core desks — core level bar + passive yield trend
    ...
    content = `
      ...
      <div class="metric-row" style="padding:4px 0">
        <span>Daily Passive Yield</span>
        <span style="font-weight:700;color:var(--success)">$${dailyMRR.toLocaleString()}/day</span>
      </div>
      ${profitPoint > 0 ? `<div style="font-size:10px;color:var(--text-faint);padding:2px 0">Path to profitability: Core Lv.${profitPoint}</div>` : ''}
    `;
    ...
    content = `
      ...
      <div style="font-size:10px;color:var(--text-faint);padding:2px 0">
        Build reputation through Herald, Care, and high-quality missions. Premium scales exponentially above rep 50.
      </div>
    `;
    ...
    // Linear desks — headcount efficiency
    const revenuePerEmployee = G.agents.length > 0
      ? Math.round(G.totalRevenue / G.agents.length)
      : 0;
    content = `
...
function getCapTableCard() {
  ...
  let html = `
    ...
    <div style="display:flex;gap:10px;font-size:10px;flex-wrap:wrap">
      <span style="color:${ceoColor}">● Steward ${c.ceo}%</span>
      ${c.investors > 0 ? `<span style="color:${invColor}">● Backers ${c.investors}%</span>` : ''}
      ${c.esop > 0 ? `<span style="color:${esopColor}">● Agent Pool ${c.esop}%</span>` : ''}
    </div>
  `;
...
function getValuationMethod() {
  const gm = GROWTH_MODELS[G.companyType];
  if (!gm) return '';
  if (gm.model === 'exponential') return 'Protocol multiple (10-20x based on passive yield growth)';
