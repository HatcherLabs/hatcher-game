// ═══════════════════════════════════════════════════════════════
//  HATCHER MARKETS — HUD Tooltip Popovers
// ═══════════════════════════════════════════════════════════════
...
function getContent(tipKey) {
  ...
  switch (tipKey) {
    case 'cash': {
      ...
      return `
        <div class="hud-popover-title">💰 Vault Balance</div>
        <div class="hud-popover-desc">Your operating funds. Goes up from completed missions, goes down from daily upkeep.</div>
        ${row('Daily Yield', `$${Math.round(rev).toLocaleString()}`, '#50c878')}
        ${row('Daily Costs', `-$${Math.round(costs).toLocaleString()}`, '#e05050')}
        ${divider()}
        ${row('Agent Upkeep', `$${totalSalary}/day · ${agentCount} agents`)}
        ${row('Room Upkeep', `$${roomCount * 15}/day · ${roomCount} rooms`)}
        ${mktBudget > 0 ? row('Signal Spend', `$${mktBudget}/day`) : ''}
        ${divider()}
        ${row('Net Profit', `${profit >= 0 ? '+' : ''}$${Math.round(profit).toLocaleString()}/day`, profit >= 0 ? '#50c878' : '#e05050')}
        ${G.capTable.ceo < 100 ? `${divider()}${row('Steward Stake', `${G.capTable.ceo}%`, G.capTable.ceo >= 50 ? '#50c878' : '#e0a030')}${G.capTable.investors > 0 ? row('Backers', `${G.capTable.investors}%`, '#e07030') : ''}${G.capTable.esop > 0 ? row('Agent Pool', `${G.capTable.esop}%`, '#5090e0') : ''}${row('Founder Score', `$${getCeoNetWorth().toLocaleString()}`, G.capTable.ceo >= 50 ? '#50c878' : '#e0a030')}` : ''}
    ...
    case 'revenue': {
      ...
      return `
        <div class="hud-popover-title">📈 Total Yield</div>
        <div class="hud-popover-desc">Lifetime yield from completed missions. Higher reputation = bigger mission payouts.</div>
        ${row('Missions Completed', projectsDone)}
        ${row('Active / Queued', `${activeProjects} / ${waitingProjects}`)}
        ${divider()}
        ${row('Rep Premium', `${Math.round(premium * 100)}%`, premium > 1 ? '#50c878' : '')}
        ${row('Daily Yield', `$${Math.round(rev).toLocaleString()}/day`, '#50c878')}
      `;
    }
    ...
    case 'day': {
      ...
      return `
        ...
        <div class="hud-popover-desc">Week ${weeksIn}, Day ${weekDay}/7. Costs are charged daily. Yield reports are weekly.</div>
        ${row('Week', weeksIn)}
    ...
    case 'rep': {
      ...
      return `
        ...
        <div class="hud-popover-desc">Your hub reputation. Higher rep attracts more market flow and increases mission payouts.</div>
        ...
        <div class="hud-popover-desc" style="margin:0;font-size:10px">
          +2 rep from ⭐ quality missions<br>
          +1 rep from ✓ decent missions<br>
    ...
    case 'funnel': {
      ...
      return `
        <div class="hud-popover-title">👁 Signal Flow</div>
        <div class="hud-popover-desc">How visitors become missions. Signal spend and reputation drive traffic. Deal rooms convert leads into market flow.</div>
        ...
        ${row('🤝 Missions/day', (m.clients || 0).toFixed(1))}
        ${divider()}
        ${row('Conv. Rate', `${Math.round(flowRate * 100)}%`, hasSales ? '#50c878' : '#e0a030')}
        ${!hasSales ? `<div class="hud-popover-desc" style="margin:4px 0 0;font-size:10px;color:#e0a030">⚠ No Deal Desk — only ${Math.round(ECONOMY.no_sales_flow_rate * 100)}% of leads convert</div>` : ''}
        ${mktBudget > 0 ? row('Signal Spend', `$${mktBudget}/day`) : '<div class="hud-popover-desc" style="margin:4px 0 0;font-size:10px;color:var(--text-faint)">💡 Set signal spend to boost paid visitors</div>'}
        ${divider()}
        <div class="hud-popover-title" style="font-size:12px;margin-top:2px">💰 AVG — Average Mission Value</div>
        <div class="hud-popover-desc">Expected payout per mission based on your current multipliers.</div>
        ${row('AVG', `$${acv.toLocaleString()}`, '#50c878')}
    ...
    case 'alignment': {
      ...
      return `
        <div class="hud-popover-title">🎯 Crew Sync</div>
        <div class="hud-popover-desc">How well your agents understand the hub strategy. Newly hatched agents start low. Weekly syncs and steward sessions improve alignment.</div>
        ${row('Average', `${avgPct}%`, avgPct > 60 ? '#50a8e8' : avgPct > 35 ? '#e0a030' : '#e05050')}
        ${row('Crew Size', nonCeo.length)}
        ...
        ${divider()}
        <div class="hud-popover-desc" style="margin:0;font-size:10px">
          🤝 Weekly syncs: +${Math.round(0.08 * 100)}% alignment<br>
          🎯 Steward Session: +${Math.round(0.15 * 100)}% alignment<br>
