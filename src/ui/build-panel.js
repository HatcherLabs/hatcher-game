// ═══════════════════════════════════════════════════════════════
//  HATCHER MARKETS — Build sidebar (room catalog + agent hatching)
// ═══════════════════════════════════════════════════════════════
...
function renderRoomButton(key, def) {
  ...
  const roleText = unlock.unlocked
    ? `${OFFICE_GUIDE[key]?.role || 'Utility'} · ${OFFICE_GUIDE[key]?.impact || 'Supports your hub operations.'}`
...
function renderBuildSection() {
  ...
  `;
  html += `<div class="build-category-header">🏗️ Market Rooms</div>`;
  ...
  html += `
    <div class="build-section">
      ...
      <div class="budget-control">
        ...
        <div class="budget-hint">Spend on signal relays to attract more mission flow</div>
      </div>
    </div>
  `;
...
function renderHireSection() {
  ...
  let html = `
    <div class="build-section">
      <h3>🥚 Hatch Agents <span class="hire-cost">${hrActive ? '🥚 Hatchery producing candidates' : founderBootstrap ? '🧭 Founder hatching available (first 3 agents)' : hrBuilt ? '🥚 Hatch a Keeper first' : '🏗️ Build Hatchery to recruit at scale'}</span></h3>
  ...
  for (const [key, role] of Object.entries(AGENT_ROLES)) {
    ...
    html += `
      <button class="hire-btn ${active}" data-role="${key}">
        ...
        <span class="hire-info">
          ...
          <span class="hire-salary">Base $${salary}/day${count > 0 ? ` · ${count} active` : ''}</span>
        </span>
      </button>
    `;
    ...
    if (selectedHireRole === key) {
      ...
      if (showCandidates) {
        ...
        if (!hasOffice) {
          html += `<div class="recruitment-status">🏗️ Build ${OFFICE_TYPES[role.office]?.name || 'room'} first</div>`;
        } else if (!hrBuilt) {
          html += `<div class="recruitment-status">🧭 Founder hatching finished. Build a Hatchery to keep recruiting.</div>`;
        } else if (!hrActive) {
          html += `<div class="recruitment-status">🥚 Hatch a Keeper — they'll find candidates</div>`;
        } else {
          html += `<div class="recruitment-status">⏳ Hatchery is working — candidates arrive after Hatch Cycles</div>`;
        }
        html += `</div>`;
      }
    }
...
function showOfficePopover(btn, key, def, guide) {
  ...
  if (guide.projects) {
    html += `
      <div class="office-popover-section">
        <div class="office-popover-label">Missions</div>
