// ═══════════════════════════════════════════════════════════════
//  HATCHER MARKETS — Right panel (missions, agents, rooms tabs)
// ═══════════════════════════════════════════════════════════════
...
function getNoProjectsDiagnosis() {
  if (G.agents.length === 0) {
    return 'No active missions — hatch your first agent.';
  }
  ...
  if (!hasTrafficSource) {
    return 'No active missions — build Signal Relay, Broadcast Studio, or Deal Desk.';
  }

  const staffCount = G.agents.filter(a => a.roleKey !== 'ceo').length;
  if (staffCount === 0) {
    return 'No active missions — hatch delivery agents for at least one room.';
  }
  ...
  if (totalVisitors < 20) {
    return 'No active missions — flow is low. Build Signal Relay or grow organic signals.';
  }
  if (leads < 0.5) {
    return 'No active missions — conversion is weak. Improve blueprint/lore setup.';
  }
  if (salesCapacity < 0.2) {
    return 'No active missions — deal capacity is near zero. Hatch Deal Runners or add Signal Relay.';
  }
  if (clients < 0.2) {
    return 'No active missions — market flow is warming up. Wait a bit or improve launch balance.';
  }

  return 'No active missions — pipeline is active; next missions should appear soon.';
}
...
export function updateAgentPanel() {
  ...
  histEl.innerHTML = '<span style="color:var(--text-faint);font-style:italic">No missions yet</span>';
    } else {
...
export function updateUI() {
  ...
  // Agents tab — use delegated click handler (set once, survives innerHTML rebuilds)
  ...
  sl.innerHTML = G.agents.map(a => {
    ...
    const stText = a.inMeeting ? '🎯 Sync' : a.state === 'thinking' ? 'Thinking...' : a.state === 'break' ? 'Break' :
    ...
    return `<div class="staff-row" data-agent-id="${a.id}">
      ...
      <span title="Sync: ${alPct}%"><span style="display:inline-block;width:16px;height:4px;background:rgba(255,255,255,0.1);border-radius:2px;vertical-align:middle;overflow:hidden"><span style="display:block;width:${alPct}%;height:100%;background:${alColor};border-radius:2px"></span></span></span>
    ...
    financeSummary.innerHTML = `
      ...
      <div style="font-size:12px">
        ...
        <div style="display:flex;gap:12px;padding:6px 0;color:var(--text-faint);font-size:11px;justify-content:flex-end">
          <span>🥚 ${G.agents.length} agents</span>
...
function getAgentActions(agent) {
    ...
    if (!G.teamBuildingActive && G.agents.length > 1) {
      ...
      if (hasMeeting) {
        ...
        actions.push({
          ...
          icon: '🎯',
          label: 'Hub Sync',
          desc: `Sync the crew (${alignPct}% avg) · All agents stop working`,
        });
      }
    }
    return actions;
  ...
  if (office === 'hr') {
    // Hatchery can recruit for any built room that needs agents
    ...
    for (const [roleKey, roleDef] of Object.entries(AGENT_ROLES)) {
      ...
      if (getCandidatesForRole(roleKey).length === 0) {
        actions.push({
          ...
          desc: `Run hatch cycle for ${OFFICE_TYPES[roleDef.office]?.name || roleDef.office}`,
        });
      }
    }
    // Always show generic hatch cycle
    if (actions.length === 0) {
      actions.push({
        id: 'recruit:any',
        icon: '🥚',
        label: 'Hatch Cycle',
        desc: 'Search for candidates across all rooms',
      });
    }
  } else {
    // Non-Hatchery agents: find internal mission templates for their room
    ...
    for (const tpl of internalTemplates) {
      ...
      if (!hasActive) {
        actions.push({
          ...
          icon: tpl.icon,
          label: tpl.name,
          desc: `Internal mission · ${tpl.phases.join(' → ')}`,
        });
      }
    }
  }
...
function executeAgentAction(agent, actionId) {
  ...
  if (actionId.startsWith('recruit:')) {
    ...
    // Create a Hatch Cycle mission and assign it immediately
    const hiringTpl = PROJECT_TEMPLATES.find(t => t.office === 'hr' && t.name === 'Hatch Cycle');
    if (!hiringTpl) return;
