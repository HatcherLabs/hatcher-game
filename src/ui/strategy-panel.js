// ═══════════════════════════════════════════════════════════════
//  HATCHER MARKETS — Strategy Panel (desk identity + growth paths)
// ═══════════════════════════════════════════════════════════════
...
function getBusinessIdentity() {
  ...
  return { title: 'Unfocused Desk', subtitle: 'Build your first room to define your market strategy.' };
}
...
function buildMoneyDrivers() {
  ...
  for (const [catKey, cat] of Object.entries(categories)) {
    ...
    for (const key of roomsInCat) {
      ...
      if (G.remodelingOffices[key] > 0) {
        ...
        diversifyHtml = `<div style="font-size:10px;color:#50c878;margin-top:2px">⚡ Converted to Mission Flow</div>`;
      } else if (hasDiversifyBranch() && canDiversifyOffice(key)) {
        const cost = getDiversificationCost(key);
        diversifyHtml = `<button class="diversify-btn" data-office="${key}" style="font-size:10px;padding:2px 8px;margin-top:3px;cursor:pointer;background:rgba(80,200,120,0.15);border:1px solid rgba(80,200,120,0.4);border-radius:4px;color:#89dca5">🔀 Convert ($${cost.toLocaleString()})</button>`;
      }
      ...
      html += `
        <div class="strategy-row">
          ...
          <div class="strategy-row-stats">Yield ${money(earned)} · Active ${active}</div>
          ${diversifyHtml}
        </div>
      `;
    }
  ...
  if (synergy.payBonus > 0) {
    html += `<div style="font-size:10px;padding:4px 6px;margin-top:6px;background:rgba(80,200,120,0.1);border:1px solid rgba(80,200,120,0.25);border-radius:4px;color:#89dca5">🤝 Mission Synergy: ${synergy.label}</div>`;
  }

  return html || '<div class="strategy-empty">No yield drivers yet. Build your first room.</div>';
}
...
function getSourceMixSummary() {
  ...
  if (recent.length === 0) {
    return 'Last 10 sources: no completed missions yet.';
  }
...
function getBottleneckDiagnosis() {
  if (G.agents.length === 0) return { state: 'blocked', text: 'No crew yet. Hatch your first agent.' };

  const hasTrafficOffice = getRoomInstances().some(r =>
    ['content', 'sales', 'marketing', 'seo'].includes(r.typeKey)
  );
  if (!hasTrafficOffice) {
    return { state: 'blocked', text: 'No traffic source. Build Signal Relay, Broadcast Studio, or Deal Desk.' };
  }

  const staffCount = G.agents.filter(a => a.roleKey !== 'ceo').length;
  if (staffCount === 0) {
    return { state: 'blocked', text: 'No mission agents. Hatch specialists for built rooms.' };
  }

  if ((G.metrics.totalVisitors || 0) < 20) {
    return { state: 'warn', text: 'Traffic bottleneck. Build Signal Relay or improve organic signals.' };
  }
  if ((G.metrics.leads || 0) < 0.5) {
    return { state: 'warn', text: 'Lead bottleneck. Improve lore/blueprint conversion quality.' };
  }
  if ((G.metrics.salesCapacity || 0) < 0.2) {
    return { state: 'warn', text: 'Deal bottleneck. Hatch Deal Runners or increase signal leverage.' };
  }
  if ((G.metrics.clients || 0) < 0.2) {
    return { state: 'warn', text: 'Pipeline is warming up. Keep speed up and monitor inflow.' };
  }

  return { state: 'ok', text: 'Pipeline healthy. Expect regular mission inflow.' };
}
...
export function updateStrategyPanel() {
  ...
  const balanceNote = balance < 70
    ? 'Signal bottleneck: add Deal Desk/Signal Relay or reduce mission headcount.'
    : 'Healthy signal/mission balance.';
  ...
  strategyEl.innerHTML = `
    ...
    <div class="strategy-card">
      <h4>What Makes Yield</h4>
      <div class="strategy-note">Potential flow/day: ${potential.toFixed(1)} · Deal capacity/day: ${capacity.toFixed(1)} · Actual missions/day: ${actual.toFixed(1)}</div>
      <div class="strategy-note">Signal balance: ${balance}% · ${balanceNote}</div>
  ...
  strategyEl.querySelectorAll('.diversify-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (!canDiversifyOffice(officeKey)) {
        showToast('Cannot convert this room.');
        return;
      }
      ...
      showToast(`🏗️ ${roomDef?.name || officeKey} is being converted to mission flow (${DIVERSIFICATION_CONFIG.remodelingDays} days)`);
      G.uiDirty = true;
      updateStrategyPanel();
    });
  });
