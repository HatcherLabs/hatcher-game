// ═══════════════════════════════════════════════════════════════
//  HATCHER MARKETS — External Events System
...
export const EXTERNAL_EVENTS = [
  // ── Backers & Investment ─────────────────────────
  {
    ...
    icon: '💰',
    title: 'Backer Offer — Generous Terms',
    description: 'A market backer sees potential in your Hatcher desk. They offer <b>$20,000 for 20%</b> equity. Good terms, but you lose some control.',
    category: 'investment',
    condition: () => G.day >= 14 && G.totalRevenue >= 5000 && !G.vcEquitySold,
    choices: [
      {
        label: 'Accept $20k for 20%',
        description: () => `+$20,000 cash · Steward: ${G.capTable.ceo}% → ${Math.round(G.capTable.ceo - 20)}%`,
        effect: () => {
          G.money += 20000;
          diluteCeo(20, 'investors', 'Backer — Generous Terms');
          G.vcEquitySold = true;
        },
        toast: () => `💰 Backer deal closed! +$20,000. Steward now owns ${G.capTable.ceo}%.`,
      },
      {
        label: 'Decline',
        description: () => `Keep ${G.capTable.ceo}% ownership`,
        effect: () => {},
        toast: '🤚 You turned down the backer. Full ownership retained.',
      },
    ],
  },
  {
    ...
    icon: '🦈',
    title: 'Backer Offer — Shark Deal',
    description: 'An aggressive investor offers <b>$10,000 for 50%</b> of your hub. Terrible terms, but the cash could save a struggling desk.',
    category: 'investment',
    condition: () => G.day >= 21 && G.money < 3000 && !G.vcEquitySold,
    choices: [
      {
        label: 'Accept $10k for 50%',
        description: () => `+$10,000 cash · Steward: ${G.capTable.ceo}% → ${Math.max(0, G.capTable.ceo - 50)}%`,
        effect: () => {
          G.money += 10000;
          diluteCeo(50, 'investors', 'Backer — Shark Deal');
          G.vcEquitySold = true;
        },
        toast: () => `🦈 Shark deal done. +$10,000 but steward now owns ${G.capTable.ceo}%.`,
      },
      {
        label: 'No way!',
  ...
  },
  {
    ...
    icon: '🤝',
    title: 'Agent Equity Proposal',
    description: 'A talented protocol agent says: <i>"I\'ll work for free for 2 weeks if you give me 5% equity."</i> They\'re skilled, but it dilutes ownership.',
    category: 'investment',
    condition: () => G.day >= 10 && G.agents.length >= 2 && G.capTable.ceo > 40,
    choices: [
      {
        label: 'Accept — 5% agent pool',
        description: () => `Free protocol agent for 14 days · Steward: ${G.capTable.ceo}% → ${G.capTable.ceo - 5}%`,
        effect: () => {
          diluteCeo(5, 'esop', 'Agent equity — Protocol Engineer');
          G.freeWorkerDaysLeft = 14;
        },
        toast: () => `🤝 Agent equity deal! Free protocol agent for 2 weeks. Steward owns ${G.capTable.ceo}%.`,
      },
      {
        label: 'Pass',
        description: 'No equity dilution',
        effect: () => {},
        toast: '👋 You passed on the agent equity deal.',
      },
    ],
  },
  ...
  // ── Competition & Market ─────────────────────────
  {
    id: 'new_competitor',
    icon: '⚔️',
    title: 'Rival Desk Enters Market',
    description: 'A well-funded rival desk just launched, offering lower prices. Your <b>AVG drops 15%</b> and <b>retention drops 5%</b> for the next 3 weeks.',
    category: 'market',
    condition: () => G.day >= 21 && G.totalRevenue >= 10000,
    choices: [
      {
        label: 'Upgrade mission quality ($3,000)',
        description: 'Halve the rival desk damage',
        effect: () => {
          if (G.money >= 3000) {
            G.money -= 3000;
        ...
        toast: () => G._lastEventResult === 'invested'
          ? '⚔️ You upgraded mission quality to fight the rival desk. Reduced impact.'
          : '⚔️ Not enough cash! Full rival desk impact applies.',
      },
      {
        label: 'Ride it out',
        description: 'Accept -15% AVG and -5% retention for 3 weeks',
        effect: () => {
          G.competitorDebuff = { weeksLeft: 3, acvPenalty: 0.15, retentionPenalty: 5 };
        },
        toast: '⚔️ Rival desk is undercutting you. Brace for impact!',
      },
    ],
  },
  {
    ...
    icon: '🚀',
    title: 'Market Boom!',
    description: 'The market is booming. Demand surges, giving you <b>+30% more missions</b> for the next 2 weeks. Can your agents handle the load?',
    category: 'market',
    condition: () => G.day >= 14 && G.agents.length >= 3,
    choices: [
      {
        label: 'Take on everything!',
        description: '+30% mission flow for 2 weeks',
        effect: () => {
          G.marketBoom = { weeksLeft: 2, bonus: 0.30 };
        },
        toast: '🚀 Market boom! +30% missions incoming for 2 weeks.',
      },
      {
        label: 'Cherry-pick premium only',
        description: '+10% missions but +15% pay per mission',
        effect: () => {
          G.marketBoom = { weeksLeft: 2, bonus: 0.10, payBonus: 0.15 };
        },
        toast: '🚀 Cherry-picking premium flow. +10% missions, +15% pay!',
      },
    ],
  },
  {
    id: 'client_disaster',
    icon: '😱',
    title: 'Major Trader Complaint',
    description: 'A high-profile trader posted a scathing review online. <b>Reputation -8</b> unless you invest in damage control.',
    category: 'market',
  ...
  // ── Government & Legal ───────────────────────────
  {
    ...
    description: 'A <b>$5,000 innovation grant</b> is available, but you need a <b>Compliance Desk</b> to handle the paperwork.',
    category: 'government',
    condition: () => G.day >= 14,
    choices: [
      {
        label: 'Apply for the grant',
        description: () => countRoomsByType('legal') > 0 ? '+$5,000 free money!' : 'Need Compliance Desk!',
        effect: () => {
        ...
        toast: () => G._lastEventResult === 'granted'
          ...
          : '🏛️ Application rejected — no Compliance Desk to file paperwork!',
      },
      {
        label: 'Skip it',
  ...
  },
  {
    ...
    icon: '🔍',
    title: 'Tax Audit!',
    description: 'The tax authority is auditing your desk. Without a <b>Treasury</b>, you pay a <b>$3,000 fine</b>. With one, it is just a minor hassle.',
    category: 'government',
    condition: () => G.day >= 28 && G.totalRevenue >= 15000,
    choices: [
      {
        label: 'Face the audit',
        description: () => countRoomsByType('finance') > 0 ? 'Treasury handles it — $500 fee' : '$3,000 fine!',
        effect: () => {
        ...
        toast: () => G._lastEventResult === 'finance'
          ? '🔍 Treasury handled the audit. -$500 admin fee.'
          : '🔍 Audit failed! -$3,000 fine. Consider building a Treasury.',
      },
      {
  ...
  // ── Talent & People ──────────────────────────────
  {
    id: 'poaching_attempt',
    icon: '🎯',
    title: 'Rival Desk Poaching Your Star!',
    description: () => {
      const star = getBestAgent();
      return star
        ? `A rival desk is offering <b>${star.name}</b> (your best performer) a 50% raise to leave. Counter-offer or lose them?`
        : 'A rival desk wants to poach one of your agents.';
    },
    category: 'talent',
    ...
    choices: [
      {
        ...
        toast: () => {
          ...
          return star
            ...
            : '🎯 You retained your agent with a raise.';
        },
      },
      {
        label: 'Let them go',
        ...
        toast: () => {
          const star = getBestAgent();
          return star
            ? `👋 ${star.name} left for the rival desk. -2 reputation.`
            : '👋 Lost an agent to the competition.';
        },
      },
    ],
  },
  {
    ...
    icon: '😤',
    title: 'Agent Morale Crisis',
    description: 'Burnout is spreading. Multiple agents are unhappy. You can run a hub reset or push through.',
    category: 'talent',
    condition: () => G.agents.filter(a => a.mood < 0.4).length >= 2,
    choices: [
      {
        label: 'Run a hub reset 🎉',
        ...
        toast: '🎉 Hub reset complete. Everyone feels better. -$1,500.',
      },
      {
        label: 'Tough it out',
  ...
  // ── Opportunities ────────────────────────────────
  {
    id: 'viral_moment',
    icon: '📱',
    title: 'A Market Run Went Viral!',
    description: 'A recent mission got shared across trading feeds. You get tons of attention: <b>+10 reputation</b> and <b>double organic traffic</b> for 1 week.',
    ...
    choices: [
      {
        ...
        toast: () => G._lastEventResult === 'amplified'
          ? '📱 Viral run amplified! +15 rep, 2x organic traffic for 1 week.'
          : '📱 Mission went viral! +10 rep, 2x organic traffic for 1 week.',
      },
      {
        ...
        toast: '📱 Viral run! +10 reputation, double organic traffic.',
      },
    ],
  },
  {
    ...
    description: 'A complementary desk wants to partner. They send you <b>3 guaranteed premium missions</b>, but take a <b>25% referral cut</b> on each.',
    category: 'opportunity',
    condition: () => G.day >= 21 && G.reputation >= 35,
    choices: [
      {
        label: 'Accept partnership',
        description: '3 premium missions (75% pay each)',
        effect: () => {
          G.partnershipProjects = 3;
        },
        toast: '🤝 Partnership deal! 3 premium missions incoming (25% referral fee).',
      },
      {
        label: 'Decline',
  ...
  },
  {
    ...
    icon: '🌊',
    title: 'Room Flood!',
    description: 'A pipe burst. One of your rooms is flooded. Repairs cost <b>$2,000</b> or you lose the room for 5 days.',
    category: 'disaster',
    ...
    choices: [
      ...
      },
      {
        ...
        description: 'Random room disabled for 5 days',
        effect: () => {
          // Pick a random non-hub room
        ...
        toast: '🌊 Room flooded! Waiting for it to dry out...',
      },
    ],
  },
  {
    id: 'esop_request',
    icon: '📋',
    title: 'Agents Want Ownership Pool',
    description: () => {
      const teamSize = G.agents.filter(a => a.roleKey !== 'ceo').length;
      return `Your crew of <b>${teamSize} agents</b> is asking for an ownership pool. Creating one boosts retention and motivation, but dilutes your ownership.`;
    },
    category: 'talent',
    condition: () => G.agents.filter(a => a.roleKey !== 'ceo').length >= 4 && G.capTable.esop < 10 && G.capTable.ceo > 50,
    choices: [
      {
        label: 'Create 10% agent pool',
        description: () => `Steward: ${G.capTable.ceo}% → ${G.capTable.ceo - 10}% · All agents get +motivation`,
        effect: () => {
          diluteCeo(10, 'esop', 'Agent pool creation');
          for (const a of G.agents) {
            if (a.roleKey !== 'ceo') {
        ...
        toast: () => `📋 Agent pool created. Steward owns ${G.capTable.ceo}%. Agents are motivated.`,
      },
      {
        label: 'Small pool (5%)',
        description: () => `Steward: ${G.capTable.ceo}% → ${G.capTable.ceo - 5}% · Moderate morale boost`,
        effect: () => {
          diluteCeo(5, 'esop', 'Agent pool (small)');
          for (const a of G.agents) {
            if (a.roleKey !== 'ceo') {
        ...
        toast: () => `📋 Small agent pool created. Steward owns ${G.capTable.ceo}%.`,
      },
      {
        label: 'No ownership pool',
        ...
        toast: '📋 No agent pool. Motivation drops.',
      },
    ],
  },
  {
    id: 'angel_investor',
    icon: '👼',
    title: 'Seed Backer Interested',
    description: () => `A seed backer loves your Hatcher vision. They offer <b>$8,000 for 10%</b> equity and boost your reputation by +5 through their network. Steward currently owns <b>${G.capTable.ceo}%</b>.`,
    category: 'investment',
    condition: () => G.day >= 10 && G.capTable.ceo > 40 && G.capTable.investors < 40 && !G.vcEquitySold,
    choices: [
      {
        label: 'Accept seed deal',
        description: () => `+$8,000 +5 rep · Steward: ${G.capTable.ceo}% → ${G.capTable.ceo - 10}%`,
        effect: () => {
          G.money += 8000;
          G.reputation = Math.min(100, G.reputation + 5);
          diluteCeo(10, 'investors', 'Seed backer');
        },
        toast: () => `👼 Seed deal! +$8,000 +5 rep. Steward owns ${G.capTable.ceo}%.`,
      },
      {
        label: 'Decline',
        description: () => `Keep ${G.capTable.ceo}% ownership`,
        effect: () => {},
        toast: '🤚 Seed backer passed on.',
      },
    ],
  },
  {
    id: 'acqui_hire_offer',
    icon: '🏢',
    title: 'Network Acquisition Offer!',
    description: () => {
      const val = getValuationForEvent();
      return `A larger desk wants to <b>acquire your agent network</b>. They are offering <b>$${Math.round(val * 0.6).toLocaleString()}</b> (60% of your $${val.toLocaleString()} valuation). Your steward stake (${G.capTable.ceo}%) would be worth <b>$${Math.round(val * 0.6 * G.capTable.ceo / 100).toLocaleString()}</b>.`;
    },
    category: 'investment',
    ...
    choices: [
      {
        ...
        description: () => {
          ...
          return `Steward gets $${payout.toLocaleString()} · Game ends`;
        },
        effect: () => {
          const val = getValuationForEvent();
          const payout = Math.round(val * 0.6 * G.capTable.ceo / 100);
        ...
        toast: () => `🏢 Network sale! Steward banked $${(G._acquiHirePayout || 0).toLocaleString()}. Hub continues under new ownership.`,
      },
      {
        label: 'Keep building',
  ...
  },
  {
    id: 'secondary_sale',
    icon: '💎',
    title: 'Secondary Stake Sale',
    description: () => `A private buyer wants to buy <b>5% of your hub</b> at current valuation. This lets you take <b>cash off the table</b> personally without diluting existing backers.`,
    category: 'investment',
    ...
    choices: [
      {
        ...
        description: () => {
          ...
          return `+$${payout.toLocaleString()} personal · Steward: ${G.capTable.ceo}% → ${G.capTable.ceo - 5}%`;
        },
        effect: () => {
          const val = getValuationForEvent();
          const payout = Math.round(val * 0.05);
          G.money += payout;
          diluteCeo(5, 'investors', 'Secondary stake sale');
        },
        toast: () => {
          const val = getValuationForEvent();
          return `💎 Sold 5% at $${val.toLocaleString()} valuation. Steward owns ${G.capTable.ceo}%.`;
        },
      },
      {
  ...
  },
  {
    ...
    icon: '💡',
    title: 'Brain Core Breakthrough',
    description: 'A new Brain Core upgrade can boost agent productivity by <b>+20%</b>, but it costs <b>$4,000</b> to implement.',
    category: 'opportunity',
    ...
    choices: [
      {
        ...
        toast: () => G._lastEventResult === 'bought'
          ? '💡 Brain Core upgrade! +20% agent efficiency for 3 weeks.'
      ...
      },
      {
        ...
        effect: () => {},
        toast: '💡 You passed on the Brain Core upgrade.',
      },
    ],
  },
];
...
export function tickEventBuffs() {
  if (G.competitorDebuff) {
    ...
    if (G.competitorDebuff.weeksLeft <= 0) {
      ...
      showToast('⚔️ Rival desk impact faded. Market normalized.');
    }
  }
  if (G.marketBoom) {
  ...
  if (G.techBuff) {
    ...
    if (G.techBuff.weeksLeft <= 0) {
      G.techBuff = null;
      showToast('💡 Brain Core boost expired.');
    }
  }
}
...
export function tickDailyEventBuffs() {
  ...
  if (G.floodedOffice) {
    ...
    if (G.floodedOffice.daysLeft <= 0) {
      showToast(`🌊 ${G.floodedOffice.office} room dried out — back in business!`);
...
function showEventModal(event) {
  ...
  event.choices.forEach((choice, idx) => {
    ...
    btn.addEventListener('click', () => {
      ...
      hideEventModal();
      G.activeEvent = null;
      G.gameSpeed = G.preEventSpeed || 0.5;
      G.uiDirty = true;
    });
...
export function getActiveBuffSummary() {
  const buffs = [];
  if (G.competitorDebuff) buffs.push(`⚔️ Rival Desk: -${Math.round(G.competitorDebuff.acvPenalty * 100)}% AVG (${G.competitorDebuff.weeksLeft}w)`);
  if (G.marketBoom) buffs.push(`🚀 Market Boom: +${Math.round(G.marketBoom.bonus * 100)}% missions (${G.marketBoom.weeksLeft}w)`);
  ...
  if (G.freeWorkerDaysLeft > 0) buffs.push(`🤝 Free protocol agent: ${G.freeWorkerDaysLeft}d left`);
  if (G.floodedOffice) buffs.push(`🌊 ${G.floodedOffice.office} room flooded: ${G.floodedOffice.daysLeft}d`);
  return buffs;
}
